...
//        printf("Warning: Failed to convert path to UTF8\n");
    ...
    wstr = doom_malloc(sizeof(wchar_t) * wlen);
...
FILE* M_fopen(const char *filename, const char *mode)
{
    ...
    return file;
#else
    return doom_fopen(filename, mode);
#endif
}
/*
...
{ 
    ...
    savedpos = doom_ftell(handle);
    
    // jump to the end and find the length
    doom_fseek(handle, 0, SEEK_END);
    length = doom_ftell(handle);

    // go back to the old location
    doom_fseek(handle, savedpos, SEEK_SET);
...
boolean M_WriteFile(const char *name, const void *source, int length)
{
    ...
    count = doom_fwrite(source, 1, length, handle);
    doom_fclose(handle);
...
{
    return doom_sscanf(str, " 0x%x", (unsigned int *) result) == 1
        || doom_sscanf(str, " 0X%x", (unsigned int *) result) == 1
        || doom_sscanf(str, " 0%o", (unsigned int *) result) == 1
        || doom_sscanf(str, " %d", result) == 1;
}
