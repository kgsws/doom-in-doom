...
static int ParseIntParameter(const char *strparm)
{
    ...
    if (strparm[0] == '0' && strparm[1] == 'x')
        doom_sscanf(strparm+2, "%x", (unsigned int *) &parm);
    else
        doom_sscanf(strparm, "%i", &parm);
...
static void LoadDefaultCollection(default_collection_t *collection)
{
    ...
    while (!doom_feof(f))
    {
        ...
        while (strlen(strparm) > 0 && !x_isprint(strparm[strlen(strparm)-1]))
        {
...
void M_LoadDefaults (void)
{
    ...
    doom_printf("saving config in %s\n", doom_defaults.filename);
    ...
    if (i)
    {
        ...
        doom_printf("        extra configuration file: %s\n", 
...
void M_SetConfigDir(const char *dir)
{
    ...
    {
        doom_printf("Using %s for configuration and saves\n", configdir);
    }
...
/*
    ...
    if (p)
    {
        ...
        doom_printf("Save directory changed to %s.\n", savegamedir);
    }
#ifdef _WIN32
...
char *M_GetAutoloadDir(const char *iwadname)
{
    ...
    if (autoload_path == NULL || strlen(autoload_path) == 0)
    {
        ...
        if (prefdir == NULL)
        {
            doom_printf("M_GetAutoloadDir: SDL_GetPrefPath failed\n");
            return NULL;
        }
