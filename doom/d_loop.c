...
boolean singletics = true; // [kg] for Doom-in-Doom
...
static void D_Disconnected(void)
{
    ...
    doom_printf("Disconnected from server.\n");
}
...
//    settings->player_classes[0] = player_class;
    ...
    if (i > 0)
        settings->extratics = doom_atoi(myargv[i+1]);
    else
    ...
    if (i > 0)
        settings->ticdup = doom_atoi(myargv[i+1]);
    else
        settings->ticdup = 1;
...
boolean D_NonVanillaRecord(boolean conditional, const char *feature)
{
    ...
    doom_printf("Warning: Recording a demo file with a non-vanilla extension "
                             ...
                             const char *feature)
{
    ...
    {
        doom_printf("Warning: WAD contains demo with a non-vanilla extension "
               "(%s)\n", feature);
        return false;
    }

    doom_printf("Warning: Playing back a demo file with a non-vanilla extension "
           "(%s). Use -strictdemos to disable this extension.\n",
           feature);
