...
*/	wipe = false;
	...
	// [kg] fullscreen status bar
	ST_DrawerOverlay(&players[displayplayer]);
    }
...
static char *GetGameName(const char *gamename)
{
    ...
    for (i=0; i<arrlen(banners); ++i)
    {
        ...
        if (deh_sub != banners[i])
        {
            ...
            deh_gamename = doom_malloc(gamename_size);
            ...
            while (deh_gamename[0] != '\0' && x_isspace(deh_gamename[0]))
            {
                memmove(deh_gamename, deh_gamename + 1, gamename_size - 1);
            }

            while (deh_gamename[0] != '\0' && x_isspace(deh_gamename[strlen(deh_gamename)-1]))
            {
...
void PrintGameVersion(void)
{
    ...
    for (i=0; gameversions[i].description != NULL; ++i)
    {
        ...
        {
            doom_printf("Emulating the behavior of the "
                   "'%s' executable.\n", gameversions[i].description);
            break;
        }
    }
...
//    W_CheckCorrectIWAD(doom);
    ...
    if (gamevariant == bfgedition)
    {
        doom_printf("BFG Edition: Using workarounds as needed.\n");
...
//        doom_printf("Playing demo %s.\n", file);
    ...
    if (M_ParmExists("-dehlump"))
    {
        ...
        doom_printf("  loaded %i DEHACKED lumps from PWAD files.\n", loaded);
    }
...
//    NET_Init ();
    ...
    if (p)
    {
	timelimit = doom_atoi(myargv[p+1]);
    }
    ...
    if (p)
    {
        startloadgame = doom_atoi(myargv[p+1]);
    }
    else
    {
        // Not loading a game
...
//        I_AtExit(StatDump, true);
    ...
    p = M_CheckParmWithArgs("-playdemo", 1);
    if (p && W_CheckNumForName(demolumpname) >= 0) // [kg] don't fail
    {
