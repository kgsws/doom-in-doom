...
{
    ST_loadData();
	// [kg] waste of space, but new drawing needs this
    st_backing_screen = (pixel_t *) Z_Malloc(SCREENWIDTH * SCREENHEIGHT * sizeof(*st_backing_screen), PU_STATIC, 0);
}
...
static void stbar_big_number_r(int x, int y, int value, int digits)
{
	...
	{
		x -= tallnum_width;
		V_DrawPatch(x, y, tallnum[value % 10]);
		value /= 10;
		if(!value)
			return;
	}
...
void ST_DrawerOverlay(player_t *pl)
{
	uint32_t tmp;
	...
	V_DrawPatch(stbar_hp_x, stbar_y, tallpercent);

	// armor
	if(pl->armorpoints)
	{
		stbar_big_number_r(stbar_ar_x, stbar_y, pl->armorpoints, 3);
		V_DrawPatch(stbar_ar_x, stbar_y, tallpercent);
	}
	...
	for(uint32_t i = 0; i < 3; i++)
	{
		...
		{
			V_DrawPatch(SCREENWIDTH - 8, tmp, keys[i]);
			tmp += 8;
		}
	}
}
