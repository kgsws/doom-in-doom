...
void M_ReadSaveStrings(void)
{
	...
	handle = M_fopen(name, "rb");
        ...
        retval = doom_fread(&savegamestrings[i], 1, SAVESTRINGSIZE, handle);
	doom_fclose(handle);
...
#if 1
	// [kg] no exit!
	M_StartMessage("I can't let you do that ...",NULL,false);
#else
...
int M_StringWidth(const char *string)
{
    ...
    {
	c = x_toupper(string[i]) - HU_FONTSTART;
