...
static char *CleanString(char *s)
{
    ...
    while (strlen(s) > 0 && x_isspace(*strending))
    {
        *strending = '\0';
        --strending;
    }
...
static void DEH_ParseContext(deh_context_t *context)
{
    ...
    while (!DEH_HadError(context))
    {
        ...
        while (line[0] != '\0' && x_isspace(line[0]))
            ++line;
        ...
        else
        {
            ...
            else
            {
                ...
                doom_sscanf(line, "%19s", section_name);
...
int DEH_LoadFile(const char *filename)
{
    ...
    doom_printf(" loading %s\n", filename);

    context = DEH_OpenFile(filename);

    if (context == NULL)
    {
        doom_printf("DEH_LoadFile: Unable to open %s\n", filename);
        return 0;
    }
...
int DEH_LoadLump(int lumpnum, boolean allow_long, boolean allow_error)
{
    ...
    if (context == NULL)
    {
        doom_printf("DEH_LoadFile: Unable to open lump %i\n", lumpnum);
        return 0;
    }
...
int DEH_LoadLumpByName(const char *name, boolean allow_long, boolean allow_error)
{
    ...
    if (lumpnum == -1)
    {
        doom_printf("DEH_LoadLumpByName: '%s' lump not found\n", name);
        return 0;
    }
