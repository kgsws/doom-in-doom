...
void MN_LoadSlotText(void)
{
	...
	doom_free(filename);
        ...
        retval = doom_fread(&SlotText[i], 1, SLOTTEXTLEN, fp);
        doom_fclose(fp);
...
{
	// [kg] no exit!
	P_SetMessage(&players[consoleplayer], "I CAN'T LET YOU DO THAT ...", true);
	MenuActive = false;
	return false;
/*
    MenuActive = false;
...
boolean MN_Responder(event_t * event)
{
    ...
    if (!FileMenuKeySteal)
    {
        ...
        else if (charTyped != 0)
        {
            ...
            for (i = 0; i < CurrentMenu->itemCount; i++)
            {
                ...
                {
                    if (x_toupper(charTyped)
                        == x_toupper(DEH_String(CurrentMenu->items[i].text)[0]))
                    {
                        CurrentItPos = i;
                        return (true);
                    }
    ...
    else
    {
        ...
        {
            if (x_isalpha(charTyped))
            {
                *textBuffer++ = x_toupper(charTyped);
                *textBuffer = ASCII_CURSOR;
                slotptr++;
                return (true);
            }
            if (x_isdigit(charTyped) || charTyped == ' '
