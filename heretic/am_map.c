...
{
#if 1
	// [kg] no scrolling background
	uint8_t *dst = I_VideoBuffer;
...
#define DOT(xx,yy,cc) fb[(yy)+(xx)*SCREENHEIGHT]=colormaps[(cc)]
...
//        oldyyshifted = yy * 320;
    ...
    fb[oldyy + xx * SCREENHEIGHT] = colormaps[*(cc)];
...
//    fb[(f_w * (f_h + 1)) / 2] = color;  // single point for now
    fb[(f_w / 2) * SCREENHEIGHT + (f_h / 2)] = colormaps[color]; // single point for now
}
