...
deh_context_t *DEH_OpenLump(int lumpnum)
{
    ...
    context->filename = doom_malloc(9);
...
*/
    return doom_fgetc(context->stream);
}
...
int DEH_GetChar(deh_context_t *context)
{
    ...
    do
    {
        ...
        if (last_was_cr && result != '\n')
        {
            ...
            {
                case DEH_INPUT_FILE:
                    doom_ungetc(result, context->stream);
                    break;
