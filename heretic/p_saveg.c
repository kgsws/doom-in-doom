...
char *SV_Filename(int slot)
{
    ...
    filename = doom_malloc(filename_len);
...
void SV_Close(char *fileName)
{
    ...
    if (vanilla_savegame_limit && doom_ftell(SaveGameFP) > SAVEGAMESIZE)
    {
        I_Error("Savegame buffer overrun");
    }

    doom_fclose(SaveGameFP);
}
...
{
    doom_fwrite(buffer, size, 1, SaveGameFP);
}
...
{
    int retval = doom_fread(buffer, 1, size, SaveGameFP);
    if (retval != size)
    {
