...
{ 
    ...
    src = source+srcy+srcx*SCREENHEIGHT; 
    dest = dest_screen+desty+destx*SCREENHEIGHT; 
...
{ 
    ...
    col = 0;
    desttop = dest_screen + y + x * SCREENHEIGHT;
...
void V_DrawTLPatch(int x, int y, patch_t * patch)
{
    ...
    for (; col < w; x++, col++, desttop += SCREENHEIGHT)
    {
        ...
        while (column->topdelta != 0xff)
        {
            ...
            while (count--)
            {
                *dest = tinttable[*dest + (colormaps[*source++] << 8)];
                dest++;
            }
            column = (column_t *) ((byte *) column + column->length + 4);
        }
...
void V_DrawAltTLPatch(int x, int y, patch_t * patch)
{
    ...
    for (; col < w; x++, col++, desttop += SCREENHEIGHT)
    {
        ...
        while (column->topdelta != 0xff)
        {
            ...
            while (count--)
            {
                *dest = tinttable[((*dest) << 8) + colormaps[*source++]];
                dest++;
            }
            column = (column_t *) ((byte *) column + column->length + 4);
        }
...
void V_DrawShadowedPatch(int x, int y, patch_t *patch)
{
    ...
    col = 0;
    desttop = dest_screen + y + x * SCREENHEIGHT;
    desttop = dest_screen + y + 2 + (x + 2) * SCREENHEIGHT;

    w = SHORT(patch->width);
    for (; col < w; x++, col++, desttop += SCREENHEIGHT, desttop2 += SCREENHEIGHT)
    {
        column = (column_t *) ((byte *) patch + LONG(patch->columnofs[col]));
...
{
    tinttable = doom_W_CacheLumpName("TINTTAB", PU_STATIC); // [kg] load from DOS instance
}
